...
The parent writes one immutable JSON file containing the raw child outcomes.
It never classifies a ``time_key`` as start/end or a row as completed/forming.

Examples::

    # RTH comparison, repeated three times two minutes apart
    python kline_snapshot_probe.py --symbol US.AAPL --session RTH \
        --repeat 3 --repeat-delay 120

    # Explicit extended-hours comparison
    python kline_snapshot_probe.py --symbol US.AAPL --session ALL \
        --repeat 2 --repeat-delay 120
"""
...
def _child_operation(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(json.dumps({
            ...
            "raw_rows": _rows_of(payload) if hasattr(payload, "to_dict") else payload,
            "duration_seconds": time.monotonic() - started,
...
def _run_one_child(args: argparse.Namespace, operation: str, ktype: str) -> dict[str, Any]:
    ...
    try:
        ...
        try:
            child_result = json.loads(raw_stdout) if raw_stdout else None
        except json.JSONDecodeError:
            child_result = {"raw_stdout": raw_stdout}
        return {
            "completed": True,
            "timed_out": False,
