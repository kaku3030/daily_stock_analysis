...
- **PR #41** | branch `harvest/futu-kline-timestamp-semantics-p0` | head `cd1f0f0a1dbea020b8aa1064707b9dd05736ab5d` | evidence report + bounded live/snapshot/analyzer tooling IMPLEMENTED | current exact-head CI / Research Radar Tests PENDING as of this sync | current exact-head independent review PENDING after tooling changes; prior review on `7ccab96f...` is HISTORICAL ONLY | provider evidence still UNKNOWN/PARTIALLY_VERIFIED by fact | positive 15m/1h Currentness timing freeze BLOCKED pending controlled US K_15M/K_60M OpenD evidence.
...
- **PR #31** | branch `harvest/open-source-ledger-v0.1` | this document is the durable Harvest synchronization surface. Cross-Window Sync Constitution V0.1 applies.
...
- Historical PR #41 head `7ccab96f...` had green CI and accepted review, but newer evidence-tooling commits moved the head to `cd1f0f0a...`; old PASS/review is historical only until the new head completes its own validation.
...
- Key remaining work: explicit 15m/1H/calendar/corporate-actions/PIT-fundamental capability truth and provider-semantic evidence.

## Next Harvest priorities
1. Finish exact-head validation/review of PR #41 tooling, then execute/ingest controlled US Futu K_15M/K_60M provider evidence when OpenD evidence is available.
2. Continue A-share intraday capability inventory for 15m/1H and currentness evidence boundaries; reuse PR #40 lineage substrate.
3. Continue Currentness / Continuity / RecoveryCandidate harvest against frozen LiveFeed contracts.
4. Independently validate stacked Data R2 after its base is stable.
5. Keep Control Tower/main line informed of architecture-impacting findings before any implementation exceeds frozen boundaries.
