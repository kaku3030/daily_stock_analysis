...
def _candidate_pool_markdown(candidates: list[dict]) -> str:
    ...
    for status in ("active", "watching", "retired"):
        ...
        if not rows:
            continue
        lines.extend([f"## {labels[status]}", "", "| 等级 | 股票 | 研究分 | 行业 | 入选次数 | 连续未入选 | 最近入选 | 核心理由 | 主要风险 |", "|---|---|---:|---|---:|---:|---|---|---|"])
        for row in rows:
            ...
            lines.append(f"| {row.get('grade', '')} | {row.get('code', '')} {row.get('name', '')} | {float(row.get('score') or 0):.1f} | {row.get('industry', '')} | {int(row.get('selected_count') or 0)} | {int(row.get('missed_runs') or 0)} | {last_selected} | {reason} | {risk} |")
        lines.append("")
    if len(lines) == 5:
...
def _write_industry_radar(candidates: list[dict], output_dir: Path) -> None:
    ...
    (output_dir / "us_research_industry_radar.json").write_text(json.dumps(radar, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
    (output_dir / "us_research_industry_radar.md").write_text(industry_radar_markdown(radar), encoding="utf-8")


def _write_earnings_valuation_radar(candidates: list[dict], output_dir: Path) -> None:
    radar = build_earnings_valuation_radar(candidates)
    (output_dir / "us_research_earnings_valuation_radar.json").write_text(json.dumps(radar, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
    (output_dir / "us_research_earnings_valuation_radar.md").write_text(earnings_valuation_radar_markdown(radar), encoding="utf-8")


def _write_financial_change_radar(candidates: list[dict], output_dir: Path) -> None:
    radar = build_financial_change_radar(candidates)
    (output_dir / "us_research_financial_changes.json").write_text(json.dumps(radar, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
    (output_dir / "us_research_financial_changes.md").write_text(financial_change_radar_markdown(radar), encoding="utf-8")
...
def _sync_candidate_pool(payload: dict, output_dir: Path) -> None:
    try:
        ...
        change_count = change_repo.sync_run(str(payload.get("market") or "us"), str(payload.get("run_id") or ""))
        latest_changes = {row.code: financial_change_to_dict(row) for row in change_repo.list_latest("us")}
        candidates = [candidate_to_dict(record) for record in repo.list_candidates(market="us", include_retired=True)]
        for candidate in candidates:
            candidate["financial_change"] = latest_changes.get(str(candidate.get("code") or ""), {})
        (output_dir / "us_research_candidate_pool.json").write_text(json.dumps(candidates, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
        (output_dir / "us_research_candidate_pool.md").write_text(_candidate_pool_markdown(candidates), encoding="utf-8")
        _write_industry_radar(candidates, output_dir)
        _write_earnings_valuation_radar(candidates, output_dir)
        _write_financial_change_radar(candidates, output_dir)
        logger.info("Candidate pool synced: inserted=%d updated=%d aged=%d watching=%d retired=%d reactivated=%d financial_snapshots=%d financial_changes=%d", stats.inserted, stats.updated, stats.aged, stats.watching, stats.retired, stats.reactivated, stats.financial_snapshots, change_count)
    except Exception:
...
def main() -> int:
    ...
    result = screen(
        os.getenv("US_RESEARCH_STRATEGY", "us_research_priority"), market="us",
        max_output=max(1, int(os.getenv("US_RESEARCH_MAX_RESULTS", "10"))),
        use_llm=_enabled("US_RESEARCH_LLM_ENABLED", True), daily_enrich=True,
        ...
        post_analyzers=["scorecard"], config=config,
    )
