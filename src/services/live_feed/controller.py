...
class LiveFeedController:
    ...
    def peek_command_for_worker(self) -> ProviderCommand | None:
        """Return the queue head without transferring its ownership."""
        with self._command_queue_lock:
            return self._command_queue[0] if self._command_queue else None

    def acknowledge_command_for_worker(self, command: ProviderCommand) -> None:
        """Remove exactly the command accepted by a local executor handoff."""
        with self._command_queue_lock:
            if not self._command_queue or self._command_queue[0] != command:
                raise RuntimeError("worker acknowledgement does not match queue head")
